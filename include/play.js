  ...
  async play(song, message, client, filters) {
    ...
    if (!song) {
      ...
      const endembed = new MessageEmbed().setColor("#F0EAD6")
        .setAuthor(`Music Queue ended.`, "https://cdn.discordapp.com/icons/725381110476963881/3ad78e075586d8156ea058fc65b1f742.png")
    ...
    try {
      let embed = new MessageEmbed()
        ...
        .setDescription(`歌曲名稱: [\`${song.title}\`](${song.url})`)
        .addField("🩸 請求者:", `>>> ${message.author.tag}`, true)
      ...
      // .setAuthor(`Started playing: ${song.title}`,'https://cdn.discordapp.com/icons/725381110476963881/3ad78e075586d8156ea058fc65b1f742.png')
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        case "⏭":
          ...
          queue.textChannel.send(`${user} ⏩ 跳過這首歌`).catch(console.error);
          collector.stop();
          break;
        ...
        case "⏯":
          ...
          if (queue.playing) {
            ...
            queue.textChannel.send(`${user} ▶ 恢復音樂！`).catch(console.error);
          }
          break;
        ...
        case "🔇":
          ...
          if (queue.volume <= 0) {
            ...
            queue.textChannel.send(`${user} 🔊 取消音樂靜音！`).catch(console.error);
          } else {
            queue.volume = 0;
            queue.connection.dispatcher.setVolumeLogarithmic(0);
            queue.textChannel.send(`${user} 🔇 把音樂靜音了！`).catch(console.error);
          }
          break;
        ...
        case "🔉":
          ...
          queue.textChannel
            .send(`${user} 🔉 降低音量 現在音量是 ${queue.volume}%`)
            .catch(console.error);
          break;
        ...
        case "🔊":
          ...
          queue.connection.dispatcher.setVolumeLogarithmic(queue.volume / 100);
          queue.textChannel
            .send(`${user} 🔊 增加音量 現在音量是 ${queue.volume}%`)
            .catch(console.error);
          break;
        ...
        case "🔁":
          ...
          queue.textChannel.send(`循環播放 ${queue.loop ? "**on**" : "**off**"}`).catch(console.error);
          break;
        ...
        case "⏹":
          ...
          if (!canModifyQueue(member)) return;
          queue.songs = [];
          queue.textChannel.send(`${user} ⏹ 停止音樂！`).catch(console.error);
          try {
            queue.connection.dispatcher.end();
          } catch (error) {
            console.error(error);
        ...
        case "🎵":
          ...
          let nowPlaying = new MessageEmbed()
            .setAuthor('♪Now playing♪', 'https://cdn.discordapp.com/icons/725381110476963881/3ad78e075586d8156ea058fc65b1f742.png', 'http://harmonymusic.tk')
        ...
        case "📑":
          ...
          let temEmbed = new MessageEmbed()
            .setAuthor("Loading...", "https://cdn.discordapp.com/icons/725381110476963881/3ad78e075586d8156ea058fc65b1f742.png").setFooter("Lyrics")
            .setColor("#F0EAD6")
          let result = await message.channel.send(temEmbed)
          try {
            lyrics = await lyricsFinder(queue.songs[0].title, "");
            if (!lyrics) lyrics = `找不到歌詞 ${queue.songs[0].title}.`;
          } catch (error) {
            lyrics = `找不到歌詞 ${queue.songs[0].title}.`;
          }
