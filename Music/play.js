...
async execute(message, args, client) {
    ...
    try {
      ...
      if (serverQueue) {
        ...
        //send join message
        message.channel.send(new MessageEmbed().setColor("#F0EAD6")
          .setDescription(`**:notes: 已加入 \`${channel.name}\` 📄 訊息 \`#${message.channel.name}\`**`)
          .setFooter(`By: ${message.author.username}#${message.author.discriminator}`))
        //if its an url
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: 正在收尋 🔍 [\`LINK\`](${args.join(" ")})**`))
          //if not 
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: 正在收尋 🔍 \`${args.join(" ")}\`**`))
        }
    ...
    if (urlValid) {
      try {
        ...
        if (error.statusCode === 403) return attentionembed(message, "最大限度 api Key的使用 請刷新！");
        console.error(error);
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .addField("列隊中的位置", `**\`${serverQueue.songs.length - 1}\`**`, true)
    ...
    try {
      ...
      return attentionembed(message, `無法加入頻道: ${error}`);
    }
  }
};
